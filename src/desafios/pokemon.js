...
buscarPokemon.addEventListener("click", () => {
    setTimeout(() => {
        let crearPokemon = pedirPokemon().then(datos => {
            createPokemonCard(datos),
                Swal.fire({
                    icon: 'success',
                    title: 'Enhorabuena',
...
async function pedirPokemon() {
    try {
        const res = await fetch(`https://pokeapi.co/api/v2/pokemon/${obtenerPokemon.value.toLowerCase()}/`)
